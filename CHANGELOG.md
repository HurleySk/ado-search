...
- **`ado-search upload-attachment <id> <file>`** - upload a local file as an attachment to an ADO work item. Step 1 POSTs the binary to `_apis/wit/attachments` to obtain an attachment URL; Step 2 PATCHes the work item with an `AttachedFile` relation. Supports all auth methods: PAT sends bytes directly via urllib; az-cli and az-powershell use `Invoke-RestMethod -InFile` to preserve binary integrity. The work item is re-fetched into the local JSONL store after attachment. Supports `--dry-run`.
